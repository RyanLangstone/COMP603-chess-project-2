 ...
 *
 * Each column stores one player in the format Name, Wins, Losses
 *
 * Example file content Alice, 3, 1 Bob, 0, 5
 *
 * This class allows loading all players into memory and saving the updates back
 * to the file by merging new data with existing data
 *
 * @author RyanL and Yaacoub
 */
public class PlayerDB {

    public static HashMap<String, Player> loadPlayers() //This just loads all players from the database and is used in PlayerStatsFrame and also NewGameFrame
    {
        HashMap<String, Player> players = new HashMap<>(); //HashMap to load all current players

        Connection conn = ChessDatabase.getConnection();
        try (Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery("SELECT name,wins,losses FROM players ORDER BY name")) {
            while (rs.next()) {
    ...
    /* 
    Saves and updates players to the database
     */
